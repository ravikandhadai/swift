  ...
  var computed: Int {
    _read {
      ...
      case .north:
        yield stored
      case _:
        break // expected-note {{missing yield in the default case}}
      }
    } // expected-error {{accessor must yield on all paths before returning}}
    ...
    _modify {
      ...
      case .north:
        yield &stored
      case _ where stored < 0: // expected-note {{missing yield in the default case}}
        stored = 12
      case .south:
        fallthrough
      case .east:
...
struct TestYieldsInLetPatterns {
  ...
  var computed3: Int {
    _read {
      if case .north = cp  { // expected-note {{missing yield in the default case}}
        yield 0
      }
    } // expected-error {{accessor must yield on all paths before returning}}
  }
