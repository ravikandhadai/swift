...
private:
  ...
  AggregateSymbolicValue(CanType aggregateType, unsigned numElements)
...
static SymbolicValue setIndexedElement(SymbolicValue aggregate,
  ...
  if (aggregate.getKind() == SymbolicValue::Aggregate) {
    assert(aggregate.getAggregateType() == type &&
           "input type does not match the type of the aggregate");
    return SymbolicValue::getAggregate(newElts, type, allocator);
  }
