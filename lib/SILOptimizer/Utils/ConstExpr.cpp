...
//                                     SILModule &module) {
//static void getOrLinkWitnessTable(ProtocolConformanceRef confRef,
//                                  SILModule &module) {
...
//  auto *wtable = lookupOrLinkWitnessTable(conf,
//                                          true, // Deserialize if necessary.
//                                          module);
  //auto *newTable = wtable;
...
//  newTable = module.getSILLoader()->lookupWitnessTable(newTable);
  ...
  // Replace all of the above with some code refactored from
  // `visitProtocolConformance`. The following code seems to fix shared linkage.
  // How to not have this?
...
//      entry.getMethodWitness().Witness->setLinkage(linkage);
  ...
  // auto *rootConf = confRef.getConcrete()->getRootConformance();
  // auto linkage = getLinkageForProtocolConformance(rootConf, NotForDefinition);
  // auto *newTable = module.createWitnessTableDeclaration(conf, linkage);
  // newTable = module.getSILLoader()->lookupWitnessTable(newTable);
//}
