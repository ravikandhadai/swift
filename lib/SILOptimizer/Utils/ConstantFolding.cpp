...
/// This utility function tries to extract the literal text, corresponding
/// to a given FloatLiteralInst, as it appears in the source file.
/// The function handles FloatLiteralInsts generated by the constant
/// folding phase by folding floating-point initializers.
/// If the extraction is successful, 'fpStr' contains the literal text.
/// Otherwise, it contains the literal as emitted by APFloat::toString.
template<unsigned N>
static void tryExtractLiteralText(FloatLiteralInst *flitInst,
                                  SmallString<N> &fpStr) {
  ...
  while(auto *callExpr = dyn_cast<CallExpr>(expr)) {
    // multi-argument constructors are not handled as of now
...
static SILValue foldFPToIntConversion(BuiltinInst *BI,
  ...
  if(conversionToUnsigned && fpVal.isNegative() && !fpVal.isZero()) {
    ...
    if(ResultsInError.hasValue()) {
      ...
      tryExtractLiteralText(flitInst, fpStr);
