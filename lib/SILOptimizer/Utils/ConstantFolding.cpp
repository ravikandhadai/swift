...
public:
  IEEESemantics(uint8_t bits, uint8_t expBits,
                uint8_t sigBits, bool explicitIntPart) {
    bitWidth = bits;
...
bool isLossyUnderflow(APFloat srcVal, BuiltinFloatType* srcType,
  ...
  uint64_t significand = bitPattern.getLoBits(srcSem.significandBitWidth)
                                   .getZExtValue();
      ...
      if (auto floatLit = dyn_cast<FloatLiteralInst>(&I)) {
        ...
        if (EnableDiagnostics && fpVal.isInfinity()) {
          ...
          diagnose(I.getModule().getASTContext(), I.getLoc().getSourceLoc(),
                   diag::warning_float_overflows_maxbuiltin,
                   litStr, fpVal.isNegative());
        }
      }
