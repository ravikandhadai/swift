...
public:
  ...
  void getLastDestroys(SmallVectorImpl<SILInstruction *> &lastDestroys) {
    assert(arrayValue);
    ...
    // Select the destroys that are followed by a frontier instruction,
    // It is important that the copied elements are destroyed before the array
    // is destoryed. This enables other optimizations.
    for (SILInstruction *frontierInst : frontier) {
      if (frontierInst == &frontierInst->getParent()->front())
        continue;
      SILInstruction *inst = &*(--frontierInst->getIterator());
      // This must be one of our destroy instructions.
      assert(isa<DestroyValueInst>(inst));
      lastDestroys.push_back(inst);
    }
  }
...
static void unrollForEach(ArrayLiteralInfo &arrayLiteralInfo,
  ...
  SmallVector<SILInstruction *, 4> lastDestroys;
  arrayLiteralInfo.getLastDestroys(lastDestroys);
  for (SILInstruction *destroy : lastDestroys) {
