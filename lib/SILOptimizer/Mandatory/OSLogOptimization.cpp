...
class OSLogOptimization : public SILFunctionTransform {
  ...
  /// compile-time constants. Specifically, identify relevant instructions to
  /// evaluate and extract the format string as a constant.
  ...
  collectConstants(SILBasicBlock::iterator first, SILBasicBlock::iterator last,
  ...
  /// could be strings or Stdlib integer-struct values or builtin integers.
  void substituteConstants(ConstantMap &constantMap);
...
void OSLogOptimization::optimizeOSLogCall(ApplyInst *oslogCall,
  ...
  // `allocStack` of the string interpolation type passed to the log call
  // (namely, OSLogMessage) and end at the last instruction of the body of the
  ...
  if (!oslogMessageArgument) {
    diagnose(astContext, oslogCall->getLoc().getSourceLoc(),
             diag::oslog_message_argument_not_found,
             astContext.Id_OSLogMessage);
    return;
  }
  ...
  auto firstI = stringInterpolAllocInst->getIterator();
  ...
  auto lastBB = SILInliner::inlineFullApply(
                    oslogApplySite, SILInliner::InlineKind::PerformanceInline,
                    funcBuilder)
                    .second;
  auto lastI = lastBB->end();

  // Interpret the instructions from first to last and fold all compile-time
  // constants found during interpretation.
...
Optional<SymbolicValue> OSLogOptimization::collectConstants(
    ...
    ConstantMap &constantMap, ConstExprStepEvaluator &constantEvaluator) {
...
void OSLogOptimization::substituteConstants(ConstantMap &constantMap) {
