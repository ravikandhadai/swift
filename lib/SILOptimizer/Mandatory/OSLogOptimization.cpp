...
/// \return true iff replacing originalVal by foldedVal was successful. The
/// function body is left unchanged if the replacement failed.
static bool replaceAllUsesAndFixLifetimes(SILValue foldedVal,
  ...
  if (!fun->hasOwnership()) {
    ...
    if (!isa<StructExtractInst>(originalInst)) {
      return false;
    }
...
static void substituteConstants(FoldState &foldState) {
  ...
  for (SILValue constantSILValue : foldState.getConstantSILValues()) {
    ...
    bool success = replaceAllUsesAndFixLifetimes(foldedSILVal, constantSILValue,
