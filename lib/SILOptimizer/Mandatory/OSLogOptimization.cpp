...
SILValue generateCode(SymbolicValue symVal, SILType &expectedType,
  ...
  case SymbolicValue::String: {
    ...
    auto stringVal =
        symVal.getStringValue(); // Note that this is a utf8 string.
    ...
    auto *applyInst =
        builder.createApply(loc, initCall->getOperand(0), SubstitutionMap(),
                            ArrayRef<SILValue>(args), false);
    return applyInst;
  }
  default: {
    assert(false && "Symbolic value other than strings are not supported");
  }
  }
}
...
class OSLogOptimization : public SILFunctionTransform {
  ...
  void run() override {
    ...
    auto fname = demangleSymbolAsString(
        getFunction()->getName(),
    ...
    for (auto &bb : fun) {
      for (auto &inst : bb) {
        ...
        auto allocStackIter = findIteratorForInstruction(
            oslogMessageArg->getDefiningInstruction());
        ...
        ConstExprStepEvaluator stepEval(allocator, &fun);
        ...
        if (oslogMessageValue.getKind() == SymbolicValue::Address) {
          ...
          printableValue =
              oslogMessageValue.getAddressValue(accessPath)->getValue();
        }
