...
class YieldOnceCheck : public SILFunctionTransform {
  ...
  void emitDiagnostics(YieldError &error, SILFunction &fun,
    ...
    case YieldError::ReturnOnConflict: {
      ...
      // in the following example, cond_br which is the instruction to find.
      //              cond_br bb1, bb2
      //              bb1:
      ...
      if (auto *switchEnum = dyn_cast<SwitchEnumInstBase>(conflictingBranch)) {
        auto diagLoc = noYieldTarget->begin()->getLoc().getSourceLoc();

        if (switchEnum->hasDefault() &&
            switchEnum->getDefaultBB() == noYieldTarget) {
          diagnose(astCtx, diagLoc, diag::case_doesnt_yield, "default");
          return;
        }
        ...
        // Handle Optional cases specially.
        if (enumElemDecl.get() == astCtx.getOptionalNoneDecl()) {
          diagnose(astCtx, diagLoc, diag::case_doesnt_yield, "nil");
        } else if (enumElemDecl.get() == astCtx.getOptionalSomeDecl()) {
          diagnose(astCtx, diagLoc, diag::case_doesnt_yield, "non-nil");
        } else {
          diagnose(astCtx, diagLoc, diag::named_case_doesnt_yield,
      ...
      if (auto *switchValue = dyn_cast<SwitchValueInst>(conflictingBranch)) {
        if (switchValue->hasDefault() &&
          ...
          diagnose(astCtx, noYieldTarget->begin()->getLoc().getSourceLoc(),
                   diag::case_doesnt_yield, "default");
          return;
        }
        // Find the case that doesn't yield.
        ...
        diagnose(
            astCtx, noYieldTarget->begin()->getLoc().getSourceLoc(),
            diag::case_doesnt_yield,
      ...
      if (auto *tryApply = dyn_cast<TryApplyInst>(conflictingBranch)) {
        diagnose(astCtx, tryApply->getLoc().getSourceLoc(),
                 ...
                 /*non-yielding error?*/ tryApply->getErrorBB() ==
                     noYieldTarget);
        return;
      }
