...
class YieldOnceCheck : public SILFunctionTransform {
  ...
  void emitDiagnostics(DiagnosticsInfo &diagInfo,
    ...
    case DiagnosticsInfo::ReturnOnConflict: {
      ...
      // Emit an error on the return statement and add a note pointing to the
      ...
      // conflict BB. In some complex control flow involving throws and
      // labelled breaks, this conditional branch may not be the right branch
      // to blame, and there could be an even better inner branch that is
      // to blame. However, this is a pretty good approximation in most cases.
      ...
      auto fallBackDiagnostics = [&] {
        diagnose(astCtx, branchLoc.getSourceLoc(), diag::conflicting_branch);
      ...
      Stmt *branchingStmt = getBranchingSourceStmt(funDecl, branchingInst);
      if (!branchingStmt) {
