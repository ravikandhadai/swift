...
class YieldOnceCheck : public SILFunctionTransform {
  ...
  /// Data-flow analysis state associated with basic blocks.
  struct BBState {

    /// Indicates wether a basic block is encountered before seeing a yield
  ...
  struct YieldError {
    ...
    // The termination instruction to report the error.
    SILInstruction * termInst;
    // The input state when the error is encountered.
    BBState inState;

  private:
    YieldError(ErrorKind kind, SILInstruction *term, BBState state) :
      errorKind(kind), termInst(term), inState(state) { }

  public:
    static YieldError getMultipleYieldError(YieldInst *yield,
                                                 BBState state) {
    ...
    static YieldError getReturnOnConflict(ReturnInst *returnI,
                                               BBState state) {
  ...
  /// \param error out parameter that will be contain information about
  /// an error that is detected.
  /// \return the state at the exit of the basic block if it can be computed
  /// and None otherwise.
  /// TODO: handle throws.
  llvm::Optional<BBState> transferStateThroughBasicBlock(SILBasicBlock *bb,
                                                         BBState inState,
  ...
  void diagnoseYieldOnceUsage(SILFunction &fun) {
    ...
    // ReturnBeforeYield errors, which denote that no paths yield, are not
    // diagnosed until the analysis completes, in order to distinguish them
    // from ReturnOnConflict errors, which happen when some paths yeild and some don't.
  ...
  void emitDiagnostics(YieldError &error, SILFunction &fun) {
    ...
    case YieldError::ReturnOnConflict: {
      ...
      // Add a note that points to a yield instruction found.
