...
public:
  ...
  bool tryToRemoveDeadAllocation(InstructionDeleter &deleter);
...
static bool hasOnlyStoreUses(ArrayRef<PMOMemoryUse> uses) {
  for (auto &u : uses) {
    // Ignore removed instructions.
    if (u.Inst == nullptr)
      continue;
    ...
    case PMOUseKind::Assign:
      return false;
    case PMOUseKind::InitOrAssign: // This is only used for trivial values.
      continue; // These don't prevent removal.
    case PMOUseKind::Load:
      return false;
    case PMOUseKind::Initialization:
      // TODO: init_existential_addr is an initializing store, but it returns
      // the address of the initialized memory. Ideally, the uses of the
      // returned address must be included in \c uses (similar to begin_access).
    ...
    case PMOUseKind::Escape:
      ...
      //    implies that the call cannot have any side-effects. That is,
      //    the effects of the call are visible only through the return values
      //    and parameters. (This implication is explained in detail below.)
      // 2. the return value of the call is never used.
      // 3. the call has only TheMemory as its indirect result, which includes
      //    @inout as well as @out SIL parameters.
      // When all the above conditions hold, the effect of the call is fully
      // contained to its indirect result (namely \c TheMemory). Therefore, it
      // is just a point store on \c TheMemory and would not affect any other
      // memory location.
      //
      // Consider now the claim that a constant evaluable function's effects are
      // visible only through its parameters. This holds true because a
      // constant evaluable function cannot read/write global state
      // or invoke calls with observable effects e.g. system calls, barriers
      // etc. Also, while a constant evaluable function can "update" a memory
      // location reachable from its writable (@inout or @out) parameter, that
      // memory location cannot be inside an arbitrary object e.g. it cannot
      // write to an arbitrary class field or invoke a mutating function on a
      // generic parameter, because neither of which is supported
      // in the constant evaluable fragment. However, it can "update" an array
      // (note: but not an array element of arbitrary type), or a string by
      // invoking mutating methods on them, as well as update a trivial value.
      // All of those updates will not be visible through any other aliases
      // of the updated parameter, because arrays, strings and trivial types
...
static bool isTemporaryStringInterpolationAllocStack(AllocationInst *alloc) {
  AllocStackInst *allocStack = dyn_cast<AllocStackInst>(alloc);
  if (!allocStack)
    return false;
  Optional<SILDebugVariable> varInfo = allocStack->getVarInfo();
  if (!varInfo)
    return false;
  return alloc->getFunction()->getASTContext().Id_dollarInterpolation.is(
      varInfo->Name);
}
...
static bool isRemovableAutogeneratedAllocation(AllocationInst *TheMemory) {
  ...
  return TheMemory->getFunction()->isTransparent() ||
         ...
         isTemporaryStringInterpolationAllocStack(TheMemory);
}

bool AllocOptimize::tryToRemoveDeadAllocation(InstructionDeleter &deleter) {
  ...
  if (MemoryType.isTrivial(*TheMemory->getFunction())) {
    ...
    deleter.forceDeleteUsers(TheMemory);
    return true;
  }
...
static bool eliminateDeadAllocations(SILFunction &fn) {
  ...
  // An object that manages deletion of instructions in a function.
  ...
  for (auto &bb : fn) {
    ...
    while (i != e) {
      ...
      AllocOptimize allocOptimize(alloc, uses, destroys, deadEndBlocks);
      changed |= allocOptimize.tryToRemoveDeadAllocation(deleter);
  ...
  // Cleanup all dead code resulting out of removing the alloc stacks.
