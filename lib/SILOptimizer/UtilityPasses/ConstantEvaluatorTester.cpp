...
class ConstantEvaluatorTester : public SILFunctionTransform {
  ...
  void run() override {
    ...
    for (auto currI = fun->getEntryBlock()->begin();;) {
      ...
      // If the instruction is marked as skip, skip it. Otherwise, try
      // evaluating the instruction and if the evaluation fails due to a
      // previously skipped instruction, skip the current instruction.
      if (shouldSkipInstruction(inst)) {
        std::tie(nextInstOpt, errorVal) = stepEvaluator.skip(currI);
      } else {
        std::tie(nextInstOpt, errorVal) =
          stepEvaluator.tryEvaluateOrElseSkip(currI);
      }

      // Diagnose errors in the evaluation. Unknown symbolic values produced
      // skipping instructions are not considered errors.
      if (errorVal.hasValue() &&
          !errorVal->isUnknownDueToSkippedInstructions()) {
